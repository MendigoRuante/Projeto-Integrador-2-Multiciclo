...
typedef enum { TipoR, TipoI, TipoJ,Dado,NOP } TipoInstrucao;
...
void controle(Memoria *instrucao, Multiciclo *multiciclo, int estado) {
  ...
  case 3:
    printf("\n\n=====Acesso a Memória LW=====\nIR: %s\nEstado: %i\n", multiciclo->IR.binario, multiciclo->Estado);
    //printf("endereço: %s\n", multiciclo->Ula.UlaSaida);
    printf("inst: %d\n", instrucao[multiciclo->Ula.UlaSaida].dado);
    printf("regis: %d\n", multiciclo->Regis_Inter);
...
void converterDadoParaBinario(Memoria *mem) {
    // Armazena fazendo casting pra manter a representação em 16 bits
    unsigned short valor = (unsigned short)mem->dado;

    for (int i = 15; i >= 0; i--) {
        int posicao = 15 - i;
        if (valor & (1 << i)) {
            mem->binario[posicao] = '1';
        } else {
            mem->binario[posicao] = '0';
        }
    }
    //Fecha a string
    mem->binario[16] = '\0';
...
void GerarMEM(Memoria *memoria){
    ...
    for(int i = 0;i<= 127;i++){
        if(memoria[i].tipo == TipoI || memoria[i].tipo == TipoJ || memoria[i].tipo == TipoR ){
            fprintf(arquivo,"%s\n",memoria[i].binario);
        }
    }
    fprintf(arquivo,".data\n");
    for(int i = 128;i <= 255; i++){
        //Pensei em iniciar tudo como NULL no campo de dado ai vereficar mas vamos manter esse provisório
        if(memoria[i].dado != 0){
